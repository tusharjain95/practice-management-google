  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      PDFDocument.prototype.font = function(name, size) {
        const regularFontPath = path.join(process.cwd(), 'public', 'fonts', 'Roboto-Regular.woff');
        const boldFontPath = path.join(process.cwd(), 'public', 'fonts', 'Roboto-Bold.woff');
        const hasCustomFonts = fs.existsSync(regularFontPath) && fs.existsSync(boldFontPath);

        if (hasCustomFonts) {
          if (!this._registeredFonts['Roboto-Regular']) {
            this.registerFont('Roboto-Regular', regularFontPath);
          }
          if (!this._registeredFonts['Roboto-Bold']) {
            this.registerFont('Roboto-Bold', boldFontPath);
          }
      ...
      const F_REGULAR = hasCustomFonts ? 'Roboto-Regular' : 'Helvetica';
      const F_BOLD = hasCustomFonts ? 'Roboto-Bold' : 'Helvetica-Bold';
