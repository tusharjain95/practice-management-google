...
export function generateRosterPdfBuffer(staffName, date, data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Title & Metadata
      doc.fillColor(PRIMARY_COLOR).fontSize(24).font('Helvetica-Bold').text('DAILY TASK ROSTER', { tracking: 1 });
      doc.fillColor(LIGHT_TEXT).fontSize(10).font('Helvetica').text(`Generated on: ${new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata' })} IST`);
      doc.moveDown(1.5);

      // Staff Header Card
      const cardY = doc.y;
      doc.rect(40, cardY, 515, 60).fillAndStroke(CARD_BG, BORDER_COLOR);
      doc.fillColor(TEXT_COLOR).fontSize(14).font('Helvetica-Bold').text('STAFF MEMBER:', 55, cardY + 15);
      doc.fontSize(16).fillColor(PRIMARY_COLOR).text(staffName.toUpperCase(), 55, cardY + 32);
      
      doc.fillColor(TEXT_COLOR).fontSize(14).font('Helvetica-Bold').text('ROSTER DATE:', 350, cardY + 15);
      doc.fontSize(16).fillColor(LIGHT_TEXT).text(date, 350, cardY + 32);
      
      doc.y = cardY + 90;

      // KPI Metrics Header
      doc.fillColor(TEXT_COLOR).fontSize(14).font('Helvetica-Bold').text('PERFORMANCE & WORKLOAD SUMMARY');
      doc.moveDown(0.5);
      ...
      kpis.forEach((kpi, idx) => {
        ...
        // Value
        doc.fillColor(kpi.color).fontSize(20).font('Helvetica-Bold').text(String(kpi.value), x + 5, kpiY + 15, { width: boxWidth - 10, align: 'center' });
        // Label
        doc.fillColor(LIGHT_TEXT).fontSize(7.5).font('Helvetica').text(kpi.label, x + 3, kpiY + 45, { width: boxWidth - 6, align: 'center' });
      });

      doc.y = kpiY + 100;

      // Productivity Summary Notes
      doc.fillColor(TEXT_COLOR).fontSize(12).font('Helvetica-Bold').text('Productivity Analysis & Focus Area');
      doc.moveDown(0.5);
      ...
      doc.moveDown(1);
      doc.fillColor(LIGHT_TEXT).fontSize(9).font('Helvetica').text('Please refer to Page 2 for a complete breakdown of all your tasks and priorities.', { align: 'center' });
      ...
      doc.fillColor(PRIMARY_COLOR).fontSize(18).font('Helvetica-Bold').text('DETAILED TASK WORKLOAD');
      doc.fillColor(LIGHT_TEXT).fontSize(10).font('Helvetica').text(`Staff: ${staffName}  |  Date: ${date}`);
      doc.moveDown(1.5);

      const renderTaskTable = (title, taskList, highlightColor = TEXT_COLOR) => {
        doc.fillColor(highlightColor).fontSize(12).font('Helvetica-Bold').text(title);
        doc.moveDown(0.4);

        if (!taskList || taskList.length === 0) {
          doc.fillColor(LIGHT_TEXT).fontSize(9).font('Helvetica-Oblique').text('No tasks in this category.');
          doc.moveDown(1.5);
          return;
        }
        ...
        taskList.forEach(task => {
          ...
          if (currentY > 740) {
            ...
            doc.rect(0, 0, 595.28, 12).fill(PRIMARY_COLOR);
            doc.y = 40;
            doc.fillColor(PRIMARY_COLOR).fontSize(12).font('Helvetica-Bold').text(`${title} (Continued)`);
            doc.moveDown(0.4);
          ...
          doc.fillColor(TEXT_COLOR).fontSize(8.5).font('Helvetica');
          ...
          doc.fillColor(prioColor).font('Helvetica-Bold').text(task.priority || 'Medium', 430, currentY + 6, { width: 50 });
          
          doc.fillColor(TEXT_COLOR).font('Helvetica').text(task.status || 'Pending', 490, currentY + 6, { width: 60 });
