...
export async function getPostgresDb(connectionString) {
  ...
  function buildSqlWhere(query, params = []) {
    ...
    for (const key of Object.keys(query)) {
      ...
      const parts = isDot ? key.split('.') : [key];
      const jsonPath = isDot ? `data#>>'{${parts.join(',')}}'` : `data->>'${key}'`;
      ...
      if (val === null || val === undefined) {
        ...
        if (isDot && parts.length === 2) {
          params.push(JSON.stringify({ [parts[0]]: [{ [parts[1]]: val }] }));
          const p2 = params.length;
          conditions.push(`(${jsonPath} = $${p1} OR data @> $${p2}::jsonb)`);
        } else if (!isDot) {
          params.push(JSON.stringify([val]));
          const p2 = params.length;
          conditions.push(`(${jsonPath} = $${p1} OR data @> $${p2}::jsonb)`);
        } else {
          conditions.push(`${jsonPath} = $${p1}`);
        }
        ...
        if (val instanceof RegExp) {
          ...
          for (const op of ops) {
            ...
            if (op === '$eq') {
              ...
              const p1 = params.length;
              if (isDot && parts.length === 2) {
                params.push(JSON.stringify({ [parts[0]]: [{ [parts[1]]: opVal }] }));
                const p2 = params.length;
                conditions.push(`(${jsonPath} = $${p1} OR data @> $${p2}::jsonb)`);
              } else {
                conditions.push(`${jsonPath} = $${p1}`);
              ...
              if (Array.isArray(opVal) && opVal.length > 0) {
                ...
                const p1 = params.length;
                if (isDot && parts.length === 2) {
                  conditions.push(`(${jsonPath} = ANY($${p1}) OR EXISTS (SELECT 1 FROM jsonb_array_elements(CASE WHEN jsonb_typeof(data->'${parts[0]}') = 'array' THEN data->'${parts[0]}' ELSE '[]'::jsonb END) elem WHERE elem->>'${parts[1]}' = ANY($${p1})))`);
                } else {
                  conditions.push(`${jsonPath} = ANY($${p1})`);
                }
              }
            } else if (op === '$nin') {
              if (Array.isArray(opVal) && opVal.length > 0) {
