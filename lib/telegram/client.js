...
<b>Matter / Nature:</b> ${task.matterType || task.title || 'Department Assistance'}
${task.noticeNo ? `<b>Notice / Ref No:</b> ${task.noticeNo}\n` : ''}<b>Client:</b> ${task.clientName || 'N/A'}
${orgName ? `<b>Organisation:</b> ${orgName}\n` : ''}<b>Priority:</b> ${task.priority || 'Medium'}
<b>Reply Due Date:</b> ${task.dueDate || 'Not set'}
${task.visitDate ? `<b>Visit / Hearing Date:</b> ${task.visitDate}\n` : ''}${task.officerDetails ? `<b>Officer / Ward:</b> ${task.officerDetails}\n` : ''}
${task.description ? `<b>Brief:</b>\n<i>${task.description}</i>\n` : ''}
...
export async function sendDepartmentReminderTelegram(db, user, task, reminderType = 'due_today') {
  ...
  const header = isTwoDaysPrior
    ? '<b>⏰ 2-DAY REMINDER: Department Matter Due Soon!</b>'
    : '<b>🚨 URGENT: Department Matter DUE TODAY!</b>';

  const subText = isTwoDaysPrior
    ? '⚠️ <i>This department notice reply or visit is due in 2 days. Please finalize submission / verify documents.</i>'
    : '🔥 <i>Today is the final due date for notice reply submission / scheduled department visit!</i>';
...
<b>Department:</b> ${task.department || 'Income Tax / GST / MCA'}
<b>Matter / Nature:</b> ${task.matterType || task.title || 'Department Assistance'}
${task.noticeNo ? `<b>Notice / Order Ref:</b> ${task.noticeNo}\n` : ''}<b>Client:</b> ${task.clientName || 'N/A'}
${orgName ? `<b>Organisation:</b> ${orgName}\n` : ''}<b>Due Date:</b> ${task.dueDate || 'Today'}
${task.visitDate ? `<b>Hearing / Visit Date:</b> ${task.visitDate}\n` : ''}${task.officerDetails ? `<b>Ward / Officer:</b> ${task.officerDetails}\n` : ''}<b>Status:</b> ${task.status || 'Pending'}
...
export async function sendDepartmentCommentTelegram(db, user, task, comment, commentatorName) {
  ...
  const text = `
<b>💬 New Note on Department Matter!</b>

<b>Department:</b> ${task.department || 'General'}
<b>Matter:</b> ${task.matterType || task.title || 'Department Assistance'}
${task.noticeNo ? `<b>Notice Ref:</b> ${task.noticeNo}\n` : ''}${orgName ? `<b>Organisation:</b> ${orgName}\n` : ''}<b>By:</b> ${commentatorName}
<b>Comment:</b>
<i>${comment.text || comment}</i>
